...
/**
 * Returns latest lts versions available in patches
 */
function latestLts() {
  const lts = ['^12.0', '^14.0', '^15.0'];

  const availableVersions = Object.keys(patchesJson);

  return lts.map((l) => semver.maxSatisfying(availableVersions, l)).filter((v) => !!v);
}
