...
export function uploadAsset (local, release, name) {
  assert(!(/[\\/]/.test(name)));
  return new Promise((resolve, reject) => {
    fs.stat(local, (error, stat) => {
      if (error) return reject(error);
      const headers = {
...
export function downloadAsset (asset, local) {
  return new Promise((resolve, reject) => {
    const headers = { Accept: 'application/octet-stream' };
    const ws = fs.createWriteStream(local);
    const url = asset.url;
    const req = request2.get(url, { headers }, (error, response, body) => {
      if (error) return reject(error);
      resolve(response);
    });
    req.pipe(ws);
